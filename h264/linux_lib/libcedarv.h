...
extern "C" {
    ...
    typedef struct CEDARV_PICTURE_INFORMATION
    {
        ...
        u8                      rotate_angle;           //*图像旋转; how this picture has been rotated, 0: no rotate, 1: 90 degree (clock wise顺时针方向), 2: 180, 3: 270, 4: horizon flip, 5: vertical flig;
        u8                      horizontal_scale_ratio; //*水平方向上按比例缩小的倍数; what ratio this picture has been scaled down at horizon size, 0: 1/1, 1: 1/2, 2: 1/4, 3: 1/8;
        u8                      vertical_scale_ratio;   //* 垂直方向上的....；what ratio this picture has been scaled down at vetical size, 0: 1/1, 1: 1/2, 2: 1/4, 3: 1/8;
        u32                     frame_rate;             //* 帧数率；frame_rate, multiplied by 1000;
        u32                     aspect_ratio;           //*像素宽与像素高比值; pixel width to pixel height ratio, multiplied by 1000;
        u32                     pict_prop;              //* 图像属性标识;picture property flags
        u8                      is_progressive;         //*逐行或隔行?; progressive or interlace picture;
        u8                      top_field_first;        //*顶部显示的第一行?; display top field first;
        ...
        cedarv_pixel_format_e   pixel_format;//*像素的格式,rgb yuv之类的 
        u64                     pts;                    //* 出现时间戳,单位为毫秒;presentation time stamp, in unit of milli-second;
        u64                     pcr;                    //* 程序时钟基准;program clock reference;

        cedarv_3d_mode_e		_3d_mode;
        cedarv_anaglath_trans_mode_e anaglath_transform_mode;//3//3d模式相关
        ...
        u8*                     y;                      //*像素数据,基于像素格式进行解读; pixel data, it is interpreted based on pixel_format;
